...
import { ArrowLeft, Search, Grid, Users } from 'lucide-react-native';
...
const ClassesScreen = ({ navigation }) => {
  ...
  const fetchClasses = async () => {
    try {
      ...
      console.log('Classes fetch error:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getTeacherName = (t) => {
    if (!t) return 'No Class Teacher';
    if (typeof t === 'string') return t;
    return `${t.first_name || ''} ${t.last_name || ''}`.trim() || 'No Class Teacher';
  };
      ...
      >
        ...
        >
          {filteredClasses.length > 0 ? filteredClasses.map((cls, idx) => (
            <View key={cls.id || idx} style={styles.card}>
              <View style={styles.cardHeader}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.cardSub}>Year: {cls.academic_year || 'N/A'}</Text>
                </View>
                <View style={styles.badge}>
                  <Text style={styles.badgeText}>{cls.sections?.length || 0} Sections</Text>
                </View>
              </View>
              
              <View style={styles.cardFooter}>
                <Users size={16} color={COLORS.slate[400]} />
                <Text style={styles.footerText}>{cls.subjects?.length || 0} Subjects Assigned</Text>
              </View>
            </View>
          )) : (
            <View style={styles.emptyContainer}>
              <Grid size={48} color={COLORS.slate[200]} />
